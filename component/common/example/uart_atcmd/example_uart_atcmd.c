...
//	flash_stream_read(&flash, UART_AT_DATA,sizeof(UART_LOG_CONF), (u8 *)&conf);
	atcmd_update_partition_info(AT_PARTITION_ATPORT, AT_PARTITION_READ, (u8 *)&conf, sizeof(UART_LOG_CONF));
	do{
...
int write_uart_atcmd_setting_to_system_data(UART_LOG_CONF* uartconf)
{
	...
	device_mutex_unlock(RT_DEV_LOCK_FLASH);
#else
	atcmd_update_partition_info(AT_PARTITION_ATPORT, AT_PARTITION_WRITE, (u8 *)uartconf, sizeof(UART_LOG_CONF));
#endif
	return 0;
}
