...
#if MCPS_REDUCE
static void coeff_get_dummy (a52_state_t * state, sample_t * coeff,
		       ...
		       sample_t level, int dither, int end)
{
	...
	default:
            bitstream_get_2 (state, bapi) ;
	    //coeff[i] = ((bitstream_get_2 (state, bapi) << (16 - bapi)) * factor[exp[i]]);
	}
    }
}
#endif
...
int a52_block (a52_state_t * state)
{
	...
	int j;
        ...
        if ((i!=0)&&(!bBlkswEn)){
        	coeff_get_dummy (state, samples + 256 * i, state->fbw_expbap +i, &quantizer,
    ...
    //a52_imdct_512 (samples, samples + 1536,
               //state->bias);

    //return 0;


    //fprintf(stderr, "i = %d, nfchans = %d\n", i, nfchans);
    if (bBlkswEn) {
	if (state->downmixed) {
	    state->downmixed = 0;
	...
	if (blksw[0]){
		a52_imdct_256 (samples, samples + 1536, state->bias);
	    }
	else 
		a52_imdct_512 (samples, samples + 1536, state->bias);
    }
