...
// optimised for use with cortex-m
//#define CPU_ARM
                             ...
                             void (*yield)(void))
{
	...
	//DiagPrintf("flac_decode_frame 0\n");
    tmp = get_bits(&s->gb, 16);
    if ((tmp & 0xFFFE) != 0xFFF8){
        return -41;
    }
	//DiagPrintf("flac_decode_frame 1\n");

    if ((framesize=decode_frame(s,decoded0,decoded1,yield)) < 0){
       s->bitstream_size=0;
       s->bitstream_index=0;
	   //DiagPrintf("flac_decode_frame 2.0, %x\n", framesize);
       return framesize;
    }
	//DiagPrintf("flac_decode_frame 2\n");
